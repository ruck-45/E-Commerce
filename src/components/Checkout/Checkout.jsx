...
export default function Checkout() {
  ...
  return (
    <Box className="p-[5rem] lg:px-32 " sx={{ width: "100%" }}>
      ...
      {activeStep === steps.length ? (
        ...
        <React.Fragment>
          <Box sx={{ display: "flex", flexDirection: "row", pt: 2 }}>
            <Button color="inherit" disabled={step == 2} onClick={handleBack} sx={{ mr: 1 }}>
              Back
            </Button>
            <Box sx={{ flex: "1 1 auto" }} />
          </Box>
          ...
          <div className="my-5">{step == 1 ? <Cart /> : step == 2 ? <AddDeliveryAddressForm /> : <OrderSummary />}</div>
