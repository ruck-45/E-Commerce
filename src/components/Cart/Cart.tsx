...
import CouponCard from "../Checkout/subComponents/CouponCard";
...
const Cart = () => {
  ...
  // State to manage the visibility of CouponCard
  const [isCouponCardOpen, setIsCouponCardOpen] = useState(false);

  const handleCouponSubmit = (couponCode: string) => {
    // Logic to handle coupon submission
    ...
    <>
      <div className="flex justify-end">
        {!isCouponCardOpen && (
          <a
            href="#"
            onClick={() => setIsCouponCardOpen(true)}
            className="text-black font-semibold text-2xl transition-colors duration-300 mr-40"
            style={{ textDecoration: "underline" }}
          >
            Click Here To Apply For Coupon Code
          </a>
        )}
      </div>
      ...
      <div className="px-[1rem] sm:px-[3rem] lg:px-0 py-[3rem]">
        {cart.length > 0 ? (
          <div className="lg:grid grid-cols-3 relative">
            ...
            <div>
              {/* Render text only if CouponCard is closed */}
              {isCouponCardOpen && (
                <div className=" pb-4">
                  <CouponCard onSubmit={handleCouponSubmit} />
                </div>
              )}

              {/* Render CouponCard based on state */}
